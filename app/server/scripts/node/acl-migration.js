if (process.argv.length !== 4) {
	console.error(
		"Takes two arguments, the API URL (like 'https://localhost/api/v1/')" +
...
async function migrate() {
	...
	await run(
		"Deleting super user from the database, if exists from a previous failed run.",
		purgeSuperUser,
		db,
		true
	);
	...
	await run(
		"Inserting super permissions in the database.",
		addSuperPermissions,
		db
	);

	await run(
		"Adding user own profile permissions in the database.",
		addSelfPermissions,
		db
	);

	const organizationEmailPairs = await run(
		"Finding organization-user pairs.",
		findOrganizationUserPairs,
		db
	);

	await run(
		"Inviting users to their organizations using API.",
		inviteUsers,
		ax,
		organizationEmailPairs
	);

	await run("Logging the super user out.", logoutSuperUser, ax);

	await run(
		"Removing super user permissions in the database.",
		removeSuperPermissions,
		db
	);
	...
	await run(
		"Finding orphans (documents without any policies).",
		findOrphans,
		db
	);
}
...
async function signUpSuperUser(ax) {
	...
	if (!signUpResponse.data.responseMeta.success) {
		...
		throw new Error(
			"Sign up failed: " + signUpResponse.data.responseMeta.error.message
		);
	}
}
...
async function loginSuperUser(ax) {
	...
	console.log(
		"Logged in. Trying to get super user profile to verify session."
	);
...
async function removeSuperPermissions(db) {
	...
	for (const [collectionName, permission] of computeSuperPermissions()) {
		promises.push(
			removePermission(db.collection(collectionName), permission)
		);
	}
	...
	async function removePermission(collection, permission) {
		...
		console.log(
			`Cleaning up empty policies for permission '${permission}'.`
		);
		await collection.updateMany(
			{ policies: { $exists: true } },
			{
				$pullAll: {
					policies: [{ permission, users: [], groups: [] }],
				},
			}
		);
	}
...
async function addSelfPermissions(db) {
	...
	while (await cursor.hasNext()) {
		...
		for (const policy of policies) {
			if (
				permissionsToAdd.delete(policy.permission) &&
				policy.users.indexOf(user.email) < 0
			) {
		...
		promises.push(
			userCollection.updateOne({ _id: user._id }, { $set: { policies } })
		);
	}
...
async function findOrganizationUserPairs(db) {
	...
	return new Promise((resolve, reject) => {
		db.collection("user")
			.find({
				organizationIds: { $exists: true },
				email: { $exists: true, $not: { $eq: SUPER_EMAIL } },
			})
			...
			.forEach(
				...
				(err) => {
					if (err === null) {
						console.info(
							`Identified ${pairs.length} invitation(s) to be sent.`
						);
						resolve(pairs);
					} else {
						console.error(err);
...
async function inviteUsers(ax, organizationEmailPairs) {
	for (const { organizationId, email } of organizationEmailPairs) {
		console.log(
			`Inviting '${email}' to organizationId: '${organizationId}'.`
		);

		const membersResponse = (
			await ax.get(`organizations/${organizationId}/members`)
		).data;
		if (
			membersResponse.responseMeta.success &&
			membersResponse.data.filter(({ username }) => username === email)
				.length > 0
		) {
			console.log(
				"User already has access to this organization. Not inviting."
			);
			continue;
		}
...
async function purgeSuperUser(db, isSilent) {
	...
	const deleteUsersResult = await db
		.collection("user")
		.deleteOne({ email: SUPER_EMAIL });
	if (!isSilent && deleteUsersResult.deletedCount !== 1) {
		throw new Error(
			"Unexpected deleted count when deleting super user: " +
				deleteUsersResult.deletedCount
		);
	}

	console.log("Deleting super user's personal organization.");
	const deleteOrgResult = await db
		.collection("organization")
		.deleteOne({ name: SUPER_EMAIL + "'s Personal Organization" });
	if (!isSilent && deleteOrgResult.deletedCount !== 1) {
		throw new Error(
			"Unexpected deleted count when deleting super user's organization: " +
				deleteOrgResult.deletedCount
		);
	}
}
...
async function runChecks(db) {
	for (const collection of await db.collections()) {
		if (
			0 !==
			(await collection.countDocuments({ "policies.users": SUPER_EMAIL }))
		) {
			console.error(
				`Super user lives on in the '${collection.collectionName}' collection.`
			);
		}
	}
}
...
async function findOrphans(db) {
	...
	for await (const datasource of cursorIterator(
		db.collection("datasource").find({
			policies: { $exists: true, $size: 0 },
		...
		if (
			organizationsInLimbo.has(datasource.organizationId) ||
			!ObjectID.isValid(datasource.organizationId)
		) {
	...
	for await (const application of cursorIterator(
		db.collection("application").find({
			policies: { $exists: true, $size: 0 },
			organizationId: { $exists: true },
			deleted: false,
		})
		...
		if (
			organizationsInLimbo.has(application.organizationId) ||
			!ObjectID.isValid(application.organizationId)
		) {
	...
	for await (const page of cursorIterator(pageCursor)) {
		if (
			applicationsInLimbo.has(page.applicationId) ||
			!ObjectID.isValid(page.applicationId)
		) {
	...
	for await (const action of cursorIterator(actionCursor)) {
		if (
			pagesInLimbo.has(action.pageId) ||
			!ObjectID.isValid(action.pageId)
		) {
	...
	console.log(
		"These are missing policies, where they shouldn've had some policies:",
		{
	...
	if (
		...
		console.error(
			"There's some documents that are missing policies, but that should've had some. Please check."
		);
	}
}
